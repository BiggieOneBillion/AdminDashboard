...
const DeleteTemplateModal = ({id }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(
        `https://api-prestigecalendar.olotusquare.co/api/v1/admin/templates/${id}`,
        {
          headers: {
      ...
      setBtnText("Done!!");
      queryClient.invalidateQueries({ queryKey: ["template_data_info_0987654", params.id] });
    } catch (error) {
      //   // console.log(error);
